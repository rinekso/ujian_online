...
class Adm extends CI_Controller {
	...
	public function m_guru() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_guru SET nama = '".bersih($p,"nama")."', nip = '".bersih($p,"nip")."', jabatan = '".bersih($p,"jabatan")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_guru VALUES (NULL,'".bersih($p,"nip")."','".bersih($p,"nama")."','".bersih($p,"jabatan")."')");
